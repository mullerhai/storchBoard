...
object TFRecordExample2 extends App {
  val logDir = "logs"
  val logFilePath = s"$logDir/train2R.tfevents"
  ...
  private def writeScalarEvent(writer: TFEventWriter, tag: String, value: Double, step: Long): Unit = {
    ...
    // 创建 Event 消息，添加 fileVersion 字段
    val event = Event(
      wallTime = System.currentTimeMillis() / 1000.0,
      step = step,
      what = Event.What.Summary(summary),
//      fileVersion = Some("brain.Event:2") // 添加 fileVersion 字段
    ).withFileVersion("brain.Event:2R").
      withSummary(summary)
  ...
  def readFromLogFile(filePath: String): Unit = {
    ...
    try {
      ...
      while (record != null) {
        val event = Event.parseFrom(record)//.withFileVersion("brain.Event:2R")
        event.what match {
