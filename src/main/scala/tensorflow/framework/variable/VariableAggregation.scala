...
sealed abstract class VariableAggregation(val value: _root_.scala.Int) extends _root_.scalapb.GeneratedEnum {
  type EnumType = VariableAggregation
  type RecognizedType = VariableAggregation.Recognized
  ...
  def companion: _root_.scalapb.GeneratedEnumCompanion[VariableAggregation] = variable.VariableAggregation
  final def asRecognized: _root_.scala.Option[VariableAggregation.Recognized] = if (isUnrecognized) _root_.scala.None else _root_.scala.Some(this.asInstanceOf[VariableAggregation.Recognized])
}
