...
sealed abstract class DataClass(val value: _root_.scala.Int) extends _root_.scalapb.GeneratedEnum {
  type EnumType = DataClass
  type RecognizedType = DataClass.Recognized
  ...
  def companion: _root_.scalapb.GeneratedEnumCompanion[DataClass] = summary.DataClass
  final def asRecognized: _root_.scala.Option[DataClass.Recognized] = if (isUnrecognized) _root_.scala.None else _root_.scala.Some(this.asInstanceOf[DataClass.Recognized])
}
