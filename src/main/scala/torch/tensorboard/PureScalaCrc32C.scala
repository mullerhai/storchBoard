...
/** Create a new PureJavaCrc32 object. */
  ...
  override def update(b: Array[Byte], offset: Int, length: Int): Unit = {
    var off = offset
    var len = length
    var localCrc = crc
    while (len > 7) {
      ...
      // 先计算右移 8 位后的值，再赋值给 localCrc
      val shiftedCrc1 = localCrc >>> 8
      localCrc = shiftedCrc1
      val c1 = b({
        off += 1;
        off - 1
      }) ^ shiftedCrc1
      // 先计算右移 8 位后的值，再赋值给 localCrc
      val shiftedCrc2 = localCrc >>> 8
      localCrc = shiftedCrc2
      val c2 = b({
        off += 1;
        off - 1
      }) ^ shiftedCrc2
      // 先计算右移 8 位后的值，再赋值给 localCrc
      val shiftedCrc3 = localCrc >>> 8
      localCrc = shiftedCrc3
      val c3 = b({
        off += 1;
        off - 1
      }) ^ shiftedCrc3
      localCrc = (PureScalaCrc32C.T8_7(c0 & 0xff) ^ PureScalaCrc32C.T8_6(c1 & 0xff)) ^ (PureScalaCrc32C.T8_5(c2 & 0xff) ^ PureScalaCrc32C.T8_4(c3 & 0xff))
      localCrc ^= (PureScalaCrc32C.T8_3(b({
        off += 1;
        off - 1
      }) & 0xff) ^ PureScalaCrc32C.T8_2(b({
        off += 1;
        off - 1
      }) & 0xff)) ^ (PureScalaCrc32C.T8_1(b({
        off += 1;
        off - 1
      }) & 0xff) ^ PureScalaCrc32C.T8_0(b({
        off += 1;
        off - 1
      }) & 0xff))
      len -= 8
    ...
    while (len > 0) {
      localCrc = (localCrc >>> 8) ^ PureScalaCrc32C.T8_0((localCrc ^ b({
        off += 1;
        off - 1
      })) & 0xff)
      len -= 1
  ...
  /** {@inheritDoc } */
  override final def update(b: Int): Unit = {
    crc = (crc >>> 8) ^ PureScalaCrc32C.T8_0((crc ^ b) & 0xff)
  }
}
